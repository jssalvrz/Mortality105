...
load("datiSplitFinal5.RData") # Dataset including estimates for Mx
...
mxSIM.fraM <- Mx.fraM[,CImx(Age = Age, mx = mx, nx=0.5, n = 863), by = list(Sex.Country)]
mxSIM.fraM$sex <- "Male"
mxSIM.fraM$country <- "France"
Mx.fraF <- data.table(subset(Mx, Sex.Country == "Females.FRA"))
mxSIM.fraF <- Mx.fraF[,CImx(Age = Age, mx = mx, nx=0.5, n = 8990), by = list(Sex.Country)]
mxSIM.fraF$sex <- "Female"
mxSIM.fraF$country <- "France"

Mx.gerM <- data.table(subset(Mx, Sex.Country == "Males.GER"))
mxSIM.gerM <- Mx.gerM[,CImx(Age = Age, mx = mx,nx=0.5, n = 111), by = list(Sex.Country)]
mxSIM.gerM$sex <- "Male"
mxSIM.gerM$country <- "Germany"
Mx.gerF <- data.table(subset(Mx, Sex.Country == "Females.GER"))
mxSIM.gerF <- Mx.gerF[,CImx(Age = Age, mx = mx, nx=0.5, n = 859), by = list(Sex.Country)]
...
mxSIM.norM <- Mx.norM[,CImx(Age = Age, mx = mx, nx=0.5, n = 41), by = list(Sex.Country)]
mxSIM.norM$sex <- "Male"
mxSIM.norM$country <- "Norway"
Mx.norF <- data.table(subset(Mx, Sex.Country == "Females.NOR"))
mxSIM.norF <- Mx.norF[,CImx(Age = Age, mx = mx, nx=0.5,n = 187), by = list(Sex.Country)]
mxSIM.norF$sex <- "Female"
mxSIM.norF$country <- "Norway"


Mx.canM <- data.table(subset(Mx, Sex.Country == "Males.CAN"))
mxSIM.canM <- Mx.canM[,CImx(Age = Age, mx = mx, nx=0.5, n = 46), by = list(Sex.Country)]
mxSIM.canM$sex <- "Male"
mxSIM.canM$country <- "Quebec"
Mx.canF <- data.table(subset(Mx, Sex.Country == "Females.CAN"))
mxSIM.canF <- Mx.canF[,CImx(Age = Age, mx = mx, nx=0.5, n = 287), by = list(Sex.Country)]
...
ggplot(subset(mxest,  sex == "Female"),
  ...
  geom_point(size = 1.7, stroke = .00001, color = "black")+
...
#ggsave("Fig2__5.pdf", width = 8, height = 6, device = cairo_pdf)
